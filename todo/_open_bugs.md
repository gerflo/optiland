...
### O6 – Überlauf-Warnungen der Asphäre bei jeder Feldänderung, ausgelöst von toten Strahlen weit außerhalb der Apertur
- **Gefunden:** 2026-09-22 (GUI-Log ~16:44 nach „Field table changes applied.“: `even_asphere.py:107/131/132: RuntimeWarning: overflow encountered in power/multiply`, `invalid value encountered in add/multiply`) · **Status:** ausgeführt
- **Ort:** `optiland/geometries/newton_raphson.py`, `_solve_distance_primal` (Newton-Schritt `f_t / safe_df_dt` ohne Begrenzung); `optiland/geometries/even_asphere.py`, `sag` und `_surface_normal`
- **Befund:** Mit `RCR-27 Beleuchtung - Asphericon OL.olsys` (Felder 2.5..4.25 mm, `optic.trace(..., 256, "hexapolar")`) laufen an der A18-Asphäre (Fläche 14, Koeffizienten bis r^14, Apertur 9 mm) je Feld 137–179 Strahlen mit endlichen Koordinaten in der Newton-Iteration davon, dazu rund 191 000 schon vorher NaN-Strahlen. Alle haben Intensität 0 (vorher abgeschattet), starten 150 mm vor der Fläche (Lochspiegel) unter 16–26° und träfen die Scheitelebene 45–75 mm neben der Achse; dort ist das Polynom so steil, dass der Newton-Schritt ins Unendliche springt und `r2 ** 7` überläuft. Die Divergenz wird über `converged` schon erkannt, lebende Strahlen und Ergebnisse sind nicht betroffen; das Log füllt sich bei jeder Feldänderung mit Warnungen, die echte Fehler verdecken.
- **Behandlung:** Berührt den Schnittpunktlöser (AGENTS.md: vor Änderungen am Kernalgorithmus fragen). Vorschlag: (a) die Iteration in `np.errstate(over="ignore", invalid="ignore")` laufen lassen (nur NumPy-Backend, Mathematik unverändert, Divergenz bleibt als nicht konvergiert markiert); Alternativen (b) Strahlen mit Intensität 0 nicht iterieren, (c) Schrittweite begrenzen – beides ändert das Verfahren. Regressionstest in `tests/`: Asphäre mit hohen Koeffizienten, ein Strahl weit außerhalb, einer innerhalb; mit `warnings.simplefilter("error")` keine Warnung, der äußere Strahl nicht konvergiert, der innere unverändert (NumPy und PyTorch).
